...
```
  ...
  Terminal atas (+), bawah (-)  Terminal atas (-), bawah (+)
  ┌─────────────────────┐      ┌─────────────────────┐
  │ D1 → ON  (forward)  │      │ D1 → OFF (reverse)  │
  │ D2 → OFF (reverse)  │      │ D2 → ON  (forward)  │
  │ D3 → OFF (reverse)  │      │ D3 → ON  (forward)  │
  │ D4 → ON  (forward)  │      │ D4 → OFF (reverse)  │
  └─────────────────────┘      └─────────────────────┘
         Arus mengalir                 Arus mengalir
      AC(+) → D1 → Beban           AC(+) → D2 → Beban
           → D4 → AC(-)                 → D3 → AC(-)
```

**Penjelasan:**
1. **Siklus Positif:** Terminal atas AC positif, bawah negatif
   - D1 dan D4 forward bias (konduksi)
   - D2 dan D3 reverse bias (tidak konduksi)
   - Arus mengalir: AC(+) → D1 → Beban → D4 → AC(-)

2. **Siklus Negatif:** Terminal atas AC negatif, bawah positif
   - D2 dan D3 forward bias (konduksi)
   - D1 dan D4 reverse bias (tidak konduksi)
   - Arus mengalir: AC(+) → D2 → Beban → D3 → AC(-)

#### C. Bentuk Gelombang
```
Input AC:    ⋂⋂⋂⋂⋂⋂⋂⋂   (Gelombang sinus)
             ~     ~     ~

Output:      ∩∩∩∩∩∩∩∩∩   (Setengah gelombang positif)
             ▔▔▔▔▔▔▔▔▔

Output:      █████████   (Setelah kapasitor filter)
             ▔▔▔▔▔▔▔▔▔   (DC mendekati rata)
```

#### D. Komponen Pendukung
| Komponen | Fungsi | Nilai Tipikal |
|----------|--------|---------------|
| Trafo Step Down | Menurunkan tegangan AC | 220V ke 12V |
| Dioda Bridge | Menyearahkan gelombang | 1N4001-1N4007 |
| Kapasitor Elektrolit | Filter riak (ripple) | 1000-4700 µF |
| Regulator Tegangan | Stabilisasi output | 7805, 7812 |
| LED Indikator | Indikasi output DC | 5mm dengan resistor |

---

### 4. Simulasi Interaktif Tinkercad

#### A. Rangkaian Dioda Bridge Online
...
#### C. Pengukuran dengan Multimeter Virtual
```markdown
